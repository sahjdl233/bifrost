					...
					<div>
						<div className="flex shrink-0 flex-col items-center pt-1 leading-none">
							...
							<div className="mt-1 flex items-center gap-1 text-[11px] text-zinc-400">
								<span>{t("dashboard.chart.ofInputTokensCachedByProvider", "input tokens cached by provider")}</span>
								<Tooltip>
									<TooltipTrigger asChild>
										<button
											...
											aria-label={t("dashboard.chart.externalCacheHitRateInfo", "More information about external cache hit rate")}
										>
									...
									<TooltipContent side="top">
										{t(
											"dashboard.chart.externalCacheHitRateDescription",
						...
						<div className="flex shrink-0 flex-wrap items-center justify-center gap-x-4 gap-y-1 pt-2 text-[11px] leading-none">
							<span className="flex items-center gap-1.5">
								...
								<span className="text-primary">
									{t("dashboard.chart.cachedTokensLabel", "Cached: {{value}}", { value: formatCompactNumber(totalCachedRead) })}
								</span>
							</span>
							<span className="flex items-center gap-1.5">
								<span className="h-2 w-2 rounded-full" style={{ backgroundColor: METER_COLORS.input }} />
								<span className="text-muted-foreground">
									{t("dashboard.chart.inputTokensLabel", "Input: {{value}}", { value: formatCompactNumber(totalPromptTokens) })}
								</span>
							</span>
						</div>
					</div>
