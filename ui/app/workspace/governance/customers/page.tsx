...
export default function GovernanceCustomersPage() {
	...
	useEffect(() => {
		...
		if (teamsError && customersError) {
			toast.error(t("governance.customers.failedToLoad", "Failed to load governance data."));
		} else {
			if (teamsError)
				toast.error(t("governance.customers.failedToLoadTeams", "Failed to load teams: {{error}}", { error: getErrorMessage(teamsError) }));
			if (customersError)
				toast.error(
					t("governance.customers.failedToLoadCustomers", "Failed to load customers: {{error}}", {
						error: getErrorMessage(customersError),
					}),
				);
		}
