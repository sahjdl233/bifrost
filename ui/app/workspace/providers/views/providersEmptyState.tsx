		...
		<div className="flex min-h-[80vh] w-full flex-col items-center justify-center gap-4 py-16 text-center">
			...
			<div className="flex flex-col gap-1">
				<h1 className="text-muted-foreground text-xl font-medium">
					{t("providers.empty.title", "Add a provider to start routing requests")}
				</h1>
				...
				<div className="mx-auto mt-6 flex flex-row flex-wrap items-center justify-center gap-2">
					<Button
						variant="outline"
						aria-label={t("providers.empty.readMoreAria", "Read more about providers (opens in new tab)")}
						data-testid="providers-button-read-more"
						onClick={() => {
							window.open(`${PROVIDERS_DOCS_URL}?utm_source=bfd`, "_blank", "noopener,noreferrer");
						}}
					>
						{t("providers.empty.readMore", "Read more")} <ArrowUpRight className="text-muted-foreground h-3 w-3" />
					</Button>
					{addProviderDropdown}
				</div>
			</div>
