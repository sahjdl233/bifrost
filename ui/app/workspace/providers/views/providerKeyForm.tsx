...
export default function ProviderKeyForm({ provider, keyId, onCancel, onSave }: Props) {
	...
	const getTooltipContent = useCallback(() => {
		if (!hasUpdateProviderAccess) {
			return t("providers.keys.noPermission", "You do not have permission to modify provider keys");
		}
		if (!form.formState.isValid && form.formState.errors.root?.message) {
			return form.formState.errors.root?.message;
		}
		if (!form.formState.isDirty) {
			return t("providers.keys.noChanges", "No changes made");
		}
		return null;
	...
	const onSubmit = (value: any) => {
		...
		mutation
			...
			.catch((err) => {
				...
				toast.error(
					isEditing ? t("providers.keys.updateError", "Error updating key") : t("providers.keys.createError", "Error creating key"),
					{
