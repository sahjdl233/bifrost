...
export function DebuggingFormFragment({ provider }: DebuggingFormFragmentProps) {
	...
	const onSubmit = (data: DebuggingFormSchema) => {
		...
		updateProvider(updatedProvider)
			.unwrap()
			.then(() => {
				toast.success(t("providers.debugging.updateSuccess", "Debugging configuration updated successfully"));
				form.reset(data);
			})
			.catch((err) => {
				toast.error(t("providers.debugging.updateFailed", "Failed to update debugging configuration"), {
		...
		<Form {...form}>
			<form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 p-6" data-testid="provider-config-debugging-content">
								...
								<div className="flex items-center justify-between space-x-2">
									<div className="space-y-0.5">
										<div className="flex items-center gap-1.5">
											<FormLabel>{t("providers.debugging.sendBackRawRequest", "Send Back Raw Request")}</FormLabel>
											<TooltipProvider>
												<Tooltip>
													...
													<TooltipContent>
														{t("providers.debugging.overridePerRequestHeader", "Override per-request with header:")}{" "}
										...
										<p className="text-muted-foreground text-xs">
											{t(
												"providers.debugging.sendBackRawRequestDescription",
						...
						render={({ field }) => (
							<FormItem>
								<div className="flex items-center justify-between space-x-2">
									<div className="space-y-0.5">
										<div className="flex items-center gap-1.5">
											<FormLabel>{t("providers.debugging.sendBackRawResponse", "Send Back Raw Response")}</FormLabel>
											<TooltipProvider>
												<Tooltip>
										...
										<p className="text-muted-foreground text-xs">
											{t(
												"providers.debugging.sendBackRawResponseDescription",
						...
						render={({ field }) => (
							<FormItem>
								<div className="flex items-center justify-between space-x-2">
									<div className="space-y-0.5">
										<div className="flex items-center gap-1.5">
											<FormLabel>{t("providers.debugging.storeRawRequestResponse", "Store Raw Request/Response")}</FormLabel>
											<TooltipProvider>
												<Tooltip>
										...
										<p className="text-muted-foreground text-xs">
											{t(
												"providers.debugging.storeRawRequestResponseDescription",
				...
				<div className="flex justify-end space-x-2">
					...
					>
						{t("providers.debugging.saveButton", "Save Debugging Configuration")}
					</Button>
				</div>
			</form>
		</Form>
