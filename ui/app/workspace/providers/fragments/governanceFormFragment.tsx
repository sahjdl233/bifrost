...
export function GovernanceFormFragment({ provider }: GovernanceFormFragmentProps) {
	...
	const saveGovernance = async (data: FormData, resetBudgetUsage: boolean) => {
		try {
			...
			toast.success(t("providers.governance.saveSuccess", "Governance configuration saved successfully"));
			form.reset(data);
		} catch (error) {
			toast.error(t("providers.governance.saveFailed", "Failed to update provider governance"), {
	...
	const handleDelete = async () => {
		try {
			...
			toast.success(t("providers.governance.removeSuccess", "Governance removed successfully"));
			form.reset(DEFAULT_GOVERNANCE_FORM_VALUES);
		} catch (error) {
			toast.error(t("providers.governance.removeFailed", "Failed to remove governance"), {
				description: getErrorMessage(error),
			});
		}
	};
			...
			<form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 px-4 md:px-6">
					...
					<div className="flex items-center justify-between gap-4">
						<div className="space-y-1">
							<Label className="text-sm" htmlFor="provider-calendar-aligned">
								{t("providers.governance.alignToCalendarCycle", "Align to calendar cycle")}
							</Label>
							<p className="text-muted-foreground text-xs">
								{t(
									"providers.governance.calendarAlignmentDescription",
				...
				<div className="space-y-4">
					<Label className="text-sm font-medium">
						{t("providers.governance.rateLimitingConfiguration", "Rate Limiting Configuration")}
					</Label>
					<NumberAndSelect
						id="providerTokenMaxLimit"
						labelClassName="font-normal"
						label={t("providers.governance.maximumTokens", "Maximum Tokens")}
					...
					<NumberAndSelect
						...
						labelClassName="font-normal"
						label={t("providers.governance.maximumRequests", "Maximum Requests")}
				...
				{hasExistingGovernance && (
					<>
						<DottedSeparator />
						<div className="space-y-4">
							<Label className="text-sm font-medium">{t("providers.governance.currentUsage", "Current Usage")}</Label>
							<div className="bg-muted/50 grid grid-cols-1 gap-4 rounded-lg p-4 md:grid-cols-2">
								{providerGovernance?.budgets?.map((b) => (
									<div key={b.id} className="space-y-1">
										<p className="text-muted-foreground text-xs">
											{t("providers.governance.budgetLabel", "Budget ({{duration}})", {
								...
								{providerGovernance?.rate_limit?.token_max_limit && (
									<div className="space-y-1">
										<p className="text-muted-foreground text-xs">{t("providers.governance.tokenUsage", "Token Usage")}</p>
										<p className="text-sm font-medium">
								...
								{providerGovernance?.rate_limit?.request_max_limit && (
									<div className="space-y-1">
										<p className="text-muted-foreground text-xs">{t("providers.governance.requestUsage", "Request Usage")}</p>
										<p className="text-sm font-medium">
				...
				<div className="mb-6 flex justify-end space-x-2">
					...
					>
						{t("providers.governance.removeConfiguration", "Remove configuration")}
					</Button>
					<Button type="submit" disabled={!form.formState.isDirty || !hasUpdateProviderAccess || isUpdating} isLoading={isUpdating}>
						{t("providers.governance.saveButton", "Save Governance Configuration")}
					</Button>
				</div>
			</form>
			<BudgetUsageResetDialog
				data-testid="provider-governance-budget-reset-dialog"
				ownerLabel={t("providers.governance.providerLabel", "provider")}
