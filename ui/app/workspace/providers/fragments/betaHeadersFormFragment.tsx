...
// i18n key for the human-readable description of each known beta header prefix.
const BETA_HEADER_DESCRIPTION_KEYS: Record<string, string> = {
	"computer-use-": "providers.betaHeaders.computerUse",
	"structured-outputs-": "providers.betaHeaders.structuredOutputs",
	"advanced-tool-use-": "providers.betaHeaders.advancedToolUse",
	"mcp-client-": "providers.betaHeaders.mcpClient",
	"prompt-caching-scope-": "providers.betaHeaders.promptCachingScope",
	"compact-": "providers.betaHeaders.compaction",
	"context-management-": "providers.betaHeaders.contextManagement",
	"files-api-": "providers.betaHeaders.filesApi",
	"interleaved-thinking-": "providers.betaHeaders.interleavedThinking",
	"skills-": "providers.betaHeaders.skills",
	"context-1m-": "providers.betaHeaders.context1m",
	"fast-mode-": "providers.betaHeaders.fastMode",
	"redact-thinking-": "providers.betaHeaders.redactThinking",
};
...
export function BetaHeadersFormFragment({ provider }: BetaHeadersFormFragmentProps) {
	...
	const onSubmit = (data: BetaHeadersFormSchema) => {
		...
		updateProvider(
			...
			.unwrap()
			.then(() => {
				toast.success(t("providers.betaHeaders.updateSuccess", "Beta header configuration updated successfully"));
				form.reset(data);
			})
			.catch((err) => {
				toast.error(t("providers.betaHeaders.updateFailed", "Failed to update beta header configuration"), {
	...
	const addCustomPrefix = useCallback(() => {
		...
		if (KNOWN_PREFIXES.has(prefix)) {
			setNewPrefixError(t("providers.betaHeaders.knownHeaderError", "This is a known header; use the override dropdown above instead"));
			return;
		}
		if (overrides[prefix] !== undefined) {
			setNewPrefixError(t("providers.betaHeaders.duplicatePrefixError", "This prefix already exists"));
			return;
		}
		if (!/^[a-z0-9-]+$/.test(prefix)) {
			setNewPrefixError(t("providers.betaHeaders.invalidPrefixError", "Prefix must contain only lowercase letters, numbers, and hyphens"));
			return;
		}
			...
			<form onSubmit={form.handleSubmit(onSubmit)} data-testid="provider-config-beta-headers-content">
				<div className="space-y-2 px-4 pb-6 md:px-6">
					<p className="text-muted-foreground text-xs">
						{t(
							"providers.betaHeaders.description",
						...
						<table className="w-full text-sm">
							<thead>
								<tr className="border-b">
									<th className="px-3 py-2 text-left font-medium">{t("providers.betaHeaders.betaHeader", "Beta Header")}</th>
									<th className="px-3 py-2 text-left font-medium">{t("providers.betaHeaders.default", "Default")}</th>
									<th className="w-[180px] px-3 py-2 text-left font-medium">{t("providers.betaHeaders.override", "Override")}</th>
								</tr>
							</thead>
							<tbody>
								{headerRows.map((row) => (
									<tr key={row.prefix} className="border-b last:border-b-0">
										<td className="px-3 py-2">
											<div className="flex flex-col gap-0.5">
												...
												<span className="text-muted-foreground text-xs">
													{t(BETA_HEADER_DESCRIPTION_KEYS[row.prefix], row.description)}
												</span>
											</div>
										</td>
										<td className="px-3 py-2">
											<Badge variant={row.defaultSupported ? "default" : "secondary"} className="text-xs">
												{row.defaultSupported
													? t("providers.betaHeaders.supported", "Supported")
													: t("providers.betaHeaders.unsupported", "Unsupported")}
											</Badge>
										</td>
										<td className="w-[180px] px-3 py-2">
											...
											>
												...
												<SelectContent>
													<SelectItem value="default">{t("providers.betaHeaders.default", "Default")}</SelectItem>
													<SelectItem value="enabled">{t("providers.betaHeaders.supported", "Supported")}</SelectItem>
													<SelectItem value="disabled">{t("providers.betaHeaders.unsupported", "Unsupported")}</SelectItem>
												</SelectContent>
											</Select>
										</td>
									</tr>
									...
									<tr key={prefix} className="border-b last:border-b-0">
										<td className="px-3 py-2">
											<div className="flex flex-col gap-0.5">
												...
												<span className="text-muted-foreground text-xs">{t("providers.betaHeaders.customHeader", "Custom header")}</span>
											</div>
										</td>
										<td className="px-3 py-2">
											<Badge variant="outline" className="text-xs">
												{t("providers.betaHeaders.custom", "Custom")}
											</Badge>
										</td>
										<td className="w-[180px] px-3 py-2">
											<div className="flex items-center gap-1">
												...
												<Button
													...
													aria-label={t("providers.betaHeaders.removePrefixAria", "Remove custom prefix {{prefix}}", { prefix })}
												>
					...
					<div className="flex items-start gap-2 pt-2">
						<div className="flex-1">
							<Input
								placeholder={t("providers.betaHeaders.addPrefixPlaceholder", "Add custom beta header prefix (e.g. new-feature-)")}
								value={newPrefix}
								onChange={(e) => {
								...
								aria-label={t("providers.betaHeaders.customPrefixAria", "Custom beta header prefix")}
				...
				<div className="bg-card sticky bottom-0 flex justify-end gap-2 rounded-b-sm border-t px-4 py-4 md:px-6">
					...
					>
						{t("providers.betaHeaders.saveButton", "Save Beta Header Configuration")}
					</Button>
				</div>
			</form>
		</Form>
