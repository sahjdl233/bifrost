		...
		>
			...
			<div className="flex flex-col gap-1">
				<h1 className="text-muted-foreground text-xl font-medium">
					{t("virtualKeys.empty.title", "Virtual keys control access, budgets, and rate limits")}
				</h1>
				<div className="text-muted-foreground mx-auto mt-2 w-full max-w-[600px] text-sm font-normal">
					{t(
						"virtualKeys.empty.subtitle",
				...
				<div className="mx-auto mt-6 flex flex-row flex-wrap items-center justify-center gap-2">
					<Button
						variant="outline"
						aria-label={t("virtualKeys.empty.readMoreAriaLabel", "Read more about virtual keys (opens in new tab)")}
						data-testid="virtual-keys-button-read-more"
						onClick={() => {
							window.open(`${VIRTUAL_KEYS_DOCS_URL}?utm_source=bfd`, "_blank", "noopener,noreferrer");
						}}
					>
						{t("virtualKeys.empty.readMore", "Read more")} <ArrowUpRight className="text-muted-foreground h-3 w-3" />
					</Button>
					<Button
						aria-label={t("virtualKeys.empty.addAriaLabel", "Add your first virtual key")}
						onClick={onAddClick}
						disabled={!canCreate}
						data-testid="create-vk-btn"
					>
						{t("virtualKeys.empty.add", "Add Virtual Key")}
					</Button>
				</div>
			</div>
		</div>
