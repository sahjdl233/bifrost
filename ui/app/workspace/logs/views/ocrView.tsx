			...
			{ocrInput && (
				<div className="w-full rounded-sm border">
					<div className="flex items-center gap-2 border-b px-6 py-2 text-sm font-medium">
						...
						{t("logs.ocr.input", "OCR Input")}
					</div>
					<div className="space-y-4 p-6">
						<div>
							<div className="text-muted-foreground mb-2 text-xs font-medium">{t("logs.ocr.type", "TYPE")}</div>
							<div className="font-mono text-xs">{ocrInput.type === "document_url" ? t("logs.ocr.document", "Document") : t("logs.ocr.image", "Image")}</div>
						</div>
						{(ocrInput.document_url || ocrInput.image_url) && (
							<div>
								<div className="text-muted-foreground mb-2 text-xs font-medium">
									{ocrInput.type === "document_url" ? t("logs.ocr.documentUrl", "DOCUMENT URL") : t("logs.ocr.imageUrl", "IMAGE URL")}
								</div>
				...
				<div className="w-full rounded-sm border">
					...
					<div className="space-y-4 p-6">
						{ocrOutput.usage_info && (
							<div className="grid grid-cols-1 gap-3 md:grid-cols-3">
								<div className="space-y-1">
									<div className="text-muted-foreground text-xs font-medium">{t("logs.ocr.pagesProcessed", "PAGES PROCESSED")}</div>
									<div className="font-mono text-xs">{ocrOutput.usage_info.pages_processed}</div>
								</div>
								<div className="space-y-1">
									<div className="text-muted-foreground text-xs font-medium">{t("logs.ocr.documentSize", "DOCUMENT SIZE")}</div>
						...
						{ocrOutput.document_annotation && (
							<div>
								<div className="text-muted-foreground mb-2 text-xs font-medium">{t("logs.ocr.documentAnnotation", "DOCUMENT ANNOTATION")}</div>
						...
						{currentPage && (
							<>
								{currentPage.dimensions && (
									<div className="grid grid-cols-1 gap-3 md:grid-cols-3">
										<div className="space-y-1">
											<div className="text-muted-foreground text-xs font-medium">{t("logs.ocr.dimensions", "DIMENSIONS")}</div>
											<div className="font-mono text-xs">
												{currentPage.dimensions.width} × {currentPage.dimensions.height}px
											</div>
										</div>
										<div className="space-y-1">
											<div className="text-muted-foreground text-xs font-medium">{t("logs.ocr.dpi", "DPI")}</div>
								...
								{currentPage.markdown ? (
									<div>
										<div className="text-muted-foreground mb-2 text-xs font-medium">{t("logs.ocr.markdown", "MARKDOWN")}</div>
										<CodeEditor
									...
									<div className="text-muted-foreground font-mono text-xs">{t("logs.ocr.noTextExtracted", "No text extracted from this page.")}</div>
								)}

								{pageImages.length > 0 && (
									<div>
										<div className="text-muted-foreground mb-2 text-xs font-medium">{t("logs.ocr.extractedImages", "EXTRACTED IMAGES ({{count}})", { count: pageImages.length })}</div>
										<div className="flex flex-wrap gap-2">
											{pageImages.map((img) => (
												<img
													key={img.id}
													src={getImageSrc(img.image_base64!)}
												alt={t("logs.ocr.imageAlt", "Image {{id}}", { id: img.id })}
								...
								{totalPages > 1 && (
									<div className="mt-3 flex items-center justify-center gap-4">
										<Button
											...
											aria-label={t("logs.ocr.previousPage", "Previous page")}
											title={t("logs.ocr.previousPage", "Previous page")}
											data-testid="ocr-view-pagination-prev-button"
										>
											<ChevronLeft className="h-4 w-4" />
										</Button>
										<span className="text-muted-foreground text-sm">
										{t("logs.ocr.pageOf", "Page {{current}} / {{total}}", { current: currentIndex + 1, total: totalPages })}
										</span>
										<Button
											variant="outline"
											size="sm"
											onClick={goToNext}
										aria-label={t("logs.ocr.nextPage", "Next page")}
										title={t("logs.ocr.nextPage", "Next page")}
